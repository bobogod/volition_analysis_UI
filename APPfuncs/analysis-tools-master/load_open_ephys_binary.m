...
if strcmp(type,'spikes')
    f=java.io.File(f.getParentFile(),fullfile(type, header.folder));
else
...
switch type
    case 'continuous'
        ...
        if (continuousmap)
            ...
            i=1;
            step=1000000;
            while i+step<length(D.Timestamps)
                t=fread(file,[header.num_channels,step],'int16');
                D.Data=[D.Data,t(17:21,:)];
                i=i+step;
            end
            step=10000;
            while i+step<length(D.Timestamps)
...
%             D.Data=fread(file,[header.num_channels Inf],'int16');
    ...
    case 'spikes'
        t=dir(string(f.getParentFile));
        for i=3:length({t.name})
            folder=[char(f.getParentFile) '\' t(i).name];
            tt=readNPY(fullfile(folder,'electrode_indices.npy'));
            tt=unique(tt);
            if ~isempty(tt)
                
                D.Timestamps{tt+1} = readNPY(fullfile(folder,'timestamps.npy'));
                D.Waveforms{tt+1} = readNPY(fullfile(folder,'waveforms.npy'));
                
                D.SortedIndexes{tt+1} = readNPY(fullfile(folder,'clusters.npy'));
                D.SampleNumbers{tt+1} = readNPY(fullfile(folder,'sample_numbers.npy'));
            end
%             D.ElectrodeIndexes{i-2} = readNPY(fullfile(folder,'electrode_indices.npy'));
        end        
    case 'events'
        D.Timestamps = readNPY(fullfile(folder,'timestamps.npy'));
