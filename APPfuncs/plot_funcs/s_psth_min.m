function s_psth_min(animal,session,Data)
%% initialization
...
for index=1:2
    ...
    t=[];
    if strcmp(align_tag,'TRIALSTART')
        for i=1:length(plots_min{2})
            t=[t;plots_min{2}{i}{2}{2}-plots_min{2}{i}{2}{1}];
        end
    elseif strcmp(align_tag,'HITTARGET')
        for i=1:length(plots_min{2})
            t=[t;plots_min{2}{i}{2}{3}-plots_min{2}{i}{2}{2}];
        end
    end
    t=[t,(1:length(t))'];
    t=sortrows(t,1);
...
set(tt,'FontSize',12);
saveas(gcf,[pwd '\FIGS\s_psth_min\' animal '-' session])
pause(2)
close(gcf)
end
