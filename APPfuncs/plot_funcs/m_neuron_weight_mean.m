function m_neuron_weight_mean(animal,DataPaths)
%% initialization
...
%                 neg_firing{index}=[neg_firing{index} (sum(spikes_n>timestamp_trialstart(i) & spikes_n<timestamp_trialstart(i)+20000)/20000*1000-mean_base_t(2,i))/std_base_t(2,i)];
    ...
    pos_mean=[pos_mean mean(pos_firing{index})]; 
    neg_mean=[neg_mean mean(neg_firing{index})];
%     scts=[scts scatter(pos_firing{index},neg_firing{index},50/(length(DataPaths)^0.333),'o','MarkerFaceColor',colors(index,:),'MarkerFaceAlpha',1/(length(DataPaths)^0.333),'MarkerEdgeAlpha',0)];
    hold on
end

scatter(pos_mean,neg_mean,[],colors,'marker','x')
plot(pos_mean,neg_mean,'k:')
...
t_color=viridis(20);
for i=1:20; line([1+i/10,1.2+i/10],[3.6,3.6],'color',t_color(i,:),'linewidth',5); end
text(1.1,3,"early",'HorizontalAlign','center','FontSize',8)
text(3.1,3,"late",'HorizontalAlign','center','FontSize',8)
text(3.1,2.6,"session",'HorizontalAlignment','center','FontSize',8)
text(1.1,2.6,"session",'HorizontalAlignment','center','FontSize',8)
text(2.5,-3.5,'max','HorizontalAlignment','center','FontSize',10)
text(-2.5,3.5,'min','HorizontalAlignment','center','FontSize',10)
set(gca,'box','on')
axis equal
...
saveas(gcf,[pwd '\FIGS\m_neuron_weight_mean\'  animal '-' DataPaths{1}(end-23:end-20) '-' DataPaths{end}(end-23:end-20)])
%     pause(5)
%     close(gcf)
end
