function m_psth_normalize(animal,DataPaths)
%% initialization
...
SELECT='min'; OFFLINE=1;
align_tags={'TRIALSTART','HITTARGET'};

f=figure;f.Color=[1,1,1];
ha=tight_subplot(1,2,[.01 .02],[.1 .1],[.1 .1],...
            ...
            if index_u>0
                ...
                for iiii=1:length(indext)
                    ...
                    if timestamp_hittarget(i)>0
                        ...
                        if 1==1%(dir(end)==0 && SELECT=="min") || (dir(end)==1 && SELECT=="max")
                            t=spikes_u;
                            align=timestamp_align(i);
                            t=t(t>align+xmin & t<align+xmax)-align; %spike lines to plot
                            if SELECT==SELECT;  plots_max{iii}{end+1}={{t},{timestamp_baselinestart(i)-align,timestamp_trialstart(i)-align,timestamp_hittarget(i)-align}}; end
%                             if SELECT=="min";  plots_min{iii}{end+1}={{t},{timestamp_baselinestart(i)-align,timestamp_trialstart(i)-align,timestamp_hittarget(i)-align}}; end
                            ...
                            mean_base{iii}=[mean_base{iii} mean_base_t(iii,i)];
                            std_base{iii}=[std_base{iii} std_base_t(iii,i)];
                        end
                    end
                end
            end
...
%to avoid 0
for i=1:length(std_base)
    for j=1:length(std_base{i})
        if std_base{i}(j)==0
            std_base{i}(j)=1;
        end
    end
    ...
    for i=1:length(units(:,1))
        ...
        if ~isempty(psths{i})
            t=(psths{i}-mean_base{i})./std_base{i};
...
%     t=[]; names={}; for i=1:length(lines); t=[t lines{i}.mainLine]; names{end+1}=['CH',num2str(units(i,1)),'U',num2str(units(i,2))]; end
    ...
    %% temp
    if strcmp(align_tag,'TRIALSTART')
        line([0,0],[-10,10],'color','g','linewidth',1)
    elseif strcmp(align_tag,'HITTARGET')
        line([0,0],[-10,10],'color','m','linewidth',1)
    end
    xlabel(ha(align_index),['time aligned to ' lower(align_tag) '/ms'])
 
    
end

ylabel(ha(1),'normalized firing rate','FontSize',10)
tt=suptitle([ animal '-' DataPaths{1}(end-23:end-20) '-' DataPaths{end}(end-23:end-20)]);
set(tt,'FontSize',12);

saveas(gcf,[pwd '\FIGS\m_psth_normalize\'  animal '-' DataPaths{1}(end-23:end-20) '-' DataPaths{end}(end-23:end-20)])
pause(5)
% close(gcf)
